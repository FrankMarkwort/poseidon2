...
namespace TestsNmea\Math\Skalar;
...
class RadTest extends TestCase
{
    ...
    {
        $rad = new Rad();
        $rad->setRad(deg2rad($actual));
        $this->assertEquals($expected, rad2deg($rad->getRad($range)), $actual);
        $this->assertEquals($rad->getDeg($range), rad2deg($rad->getRad($range)));
    }
}
